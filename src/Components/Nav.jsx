...
export default function Nav({ mostrar,cantidad }) {
  return (
    <div
      className="fixed z-50 bottom-0 right-0 flex w-full  justify-center items-center p-2 drop-shadow-2xl"
      onClick={mostrar}
    >
      <div className=" flex bg-black p-2 text-white rounded-full gap-2 hover:text-[#FF7A21] cursor-pointer">
        <div className="relative w-10 h-10 flex justify-center items-center bg-white bg-opacity-10 rounded-full">
          ...
          <div className="absolute top-0 left-7   text-[#C5150C]  font-bold">
            <div className=" text-[15px] flex justify-center items-center  rounded-full h-5 w-5 text-black bg-white font-bold">
              {cantidad}
            </div>
          </div>
        </div>
