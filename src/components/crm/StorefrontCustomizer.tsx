...
export const StorefrontCustomizer: React.FC = () => {
  const { storeConfig, updateStoreConfig, resetStoreConfig, products, setActiveView, adminTheme } =
    useRestaurant()

  // Local draft state for live interactive tweaking
  const [draft, setDraft] = useState<StorefrontConfig>(storeConfig)
  const [previewDevice, setPreviewDevice] = useState<"desktop" | "mobile">("desktop")
  const [activeSection, setActiveSection] = useState<"branding" | "colors" | "uiux" | "business">(
    "branding"
  )
  ...
  const getCardStyleClasses = (style: CardStyle, bgTheme: StoreBgTheme) => {
  ...
  const getBgStyle = (theme: StoreBgTheme) => {
    switch (theme) {
      case "dark-charcoal":
